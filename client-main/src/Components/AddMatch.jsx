      ...
      {players.map((player) => (
        ...
        >
          <input
            ...
            onChange={(e) =>
              handlePlayerSelection(player._id, e.target.checked, isHome)
            }
            className="w-4 h-4"
          />
    ...
    <div className="space-y-4">
      ...
      {selectedCountry && (
        <Select
          value={selectedTeam}
          onChange={onTeamChange}
          options={nationalTeams?.map((team) => ({
...
const [awayNationalTeams, setAwayNationalTeams] = useState([]);
      ...
      try {
        ...
        if (matchType === 'ClubTeam') {
          const response = await fetch(
            `${import.meta.env.VITE_REACT_APP_API_URL}/api/club/${teamId}/players?date=${dateParam}`
          );
          if (!response.ok) throw new Error("Failed to fetch players");
          players = await response.json();
        } else {
          const response = await fetch(
            `${import.meta.env.VITE_REACT_APP_API_URL}/api/country/national-teams/${teamId}/players?date=${dateParam}`
          );
          if (!response.ok) throw new Error("Failed to fetch players");
          players = await response.json();
        }
  ...
  }, [matchData.homeTeam.team, matchData.awayTeam.team, matchData.date, matchType, toast]);
  ...
  const checkExistingMatches = async (teamId, date, type) => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_REACT_APP_API_URL}/api/match/check-team-availability?${queryParams}`
      );
      
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || "Failed to check team availability");
      }
  ...
  const validateOdds = () => {
    ...
    if (homeWinNum < 0 || drawNum < 0 || awayWinNum < 0 ||
        homeWinNum > 1 || drawNum > 1 || awayWinNum > 1) {
      return false;
    }
  ...
  const validatePlayers = () => {
    const homeStarters = matchData.homeTeam.players.filter(p => p.starter).length;
    const awayStarters = matchData.awayTeam.players.filter(p => p.starter).length;
 ...
 if (!validateOdds()) {
  toast({
    variant: "destructive",
    description: "Match odds must be between 0 and 1, and should roughly sum to 1 (±10% margin allowed)",
  });
  return;
}
  ...
  return;
    ...
    try {
      ...
      const [homeTeamCheck, awayTeamCheck] = await Promise.all([
        checkExistingMatches(matchData.homeTeam.team, matchData.date, matchType),
        checkExistingMatches(matchData.awayTeam.team, matchData.date, matchType)
      ]);
...
>
  ...
  selectedTeam={matchData.homeTeam.team ? {
    value: matchData.homeTeam.team,
    label: matchType === 'ClubTeam'
      ? clubsData?.find(club => club._id === matchData.homeTeam.team)?.name
      : homeNationalTeams?.find(team => team._id === matchData.homeTeam.team)
          ? `${selectedHomeCountry?.label} ${homeNationalTeams.find(team => team._id === matchData.homeTeam.team).type}`
          : ''
  } : null}
...
<Card>
  ...
  isHome={false}
  matchType={matchType} // This will now receive 'ClubTeam' or 'NationalTeam'
  selectedTeam={matchData.awayTeam.team ? {
    value: matchData.awayTeam.team,
    label: matchType === 'ClubTeam'
      ? clubsData?.find(club => club._id === matchData.awayTeam.team)?.name
      : awayNationalTeams?.find(team => team._id === matchData.awayTeam.team)
          ? `${selectedAwayCountry?.label} ${awayNationalTeams.find(team => team._id === matchData.awayTeam.team).type}`
          : ''
  } : null}
