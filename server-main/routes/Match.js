...
router.get('/matches/:id', async (req, res) => {
    try {
        ...
        const match = await populateMatchData(
...
router.post('/matches', async (req, res) => {
    try {
        ...
        const totalOdds = Number(odds.homeWin) + Number(odds.draw) + Number(odds.awayWin);
        ...
        if (existingMatch) {
            ...
            const conflictingTeamId = existingMatch.homeTeam.team.toString() === req.body.homeTeam.team ||
            ...
            if (req.body.type === 'ClubTeam') {
                ...
                teamName = nationalTeam ? `${nationalTeam.country} ${nationalTeam.type}` : 'Unknown Team';
            }
        ...
        const transformedMatch = {
            ...
            homeTeam: {
                ...
                teamName: req.body.type === 'ClubTeam'
...
router.get('/check-team-availability', async (req, res) => {
    try {
        ...
        if (!teamId || !date || !type) {
            return res.status(400).json({ 
                message: 'Missing required parameters. Team ID, date, and type are required.'
            });
        }
